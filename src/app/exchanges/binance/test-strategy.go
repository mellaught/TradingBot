...
func (b *BinanceWorker) StrategyStart() {

	//change := "T"
	go func() {
		fmt.Println("Start strategy")
		for {
			t := <-b.AllMarketTickersC
			for i, s := range t {
				if s.Symbol == "BTCUSDT" {
					fmt.Println(i) 
					
					fmt.Println(s.LastPrice)
				}
			}

			// t := <-b.WsTickers.Subs["TICKER"]
...
func (p *BinanceWorker) GetProfit(first, middle, third, Fee decimal.Decimal) decimal.Decimal {
